...
- **实现模块**：`src/lportal/i18n.py` 统一负责语言检测和翻译
- **语言策略**：系统语言为中文（`zh_CN`、`zh_TW`、`zh_HK` 等）时展示中文，其余所有环境默认展示英文
- **开发规范**：
  - 所有面向用户的终端输出字符串、命令反馈、WebSocket 错误提示都必须通过 `from .i18n import _` 调用
  - 禁止在代码中直接硬编码中文或英文用户可见文本
  - 实现新需求时，必须同时在 `i18n.py` 中增加**中文**和**英文**两份翻译文本
  - 翻译键使用自然语义英文（如 `"Auto copy mode: ON"`），便于代码可读性
- **覆盖范围**：`ui.py`、`commands.py`、`main.py`、`server.py`、`beauty.py` 中所有用户可见字符串
